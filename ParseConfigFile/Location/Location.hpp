...
class Location //vector de locations en Server
{
	private:
		...
		std::map<std::string, std::string>	cgi_mix;
	...
	public:
		...
		//setters
			...
			void addCgiMix(std::string const extension, std::string const path);
		...
		//getters
			...
			const std::map<std::string, std::string> &getCgiMix() const;
			const std::string &getCgiKey(std::string key) const;
};